...
def official_evaluate_prompt(tmp, path, dev_file, sample_ratio):
    '''
        Adapted from the official evaluation code
    '''
    truth_dir = os.path.join(path, 'ref')
    ...
    if 'pkl' in dev_file:
        ck_path = os.path.join(path, dev_file) + str(sample_ratio)
        if os.path.exists(ck_path):
            truth = pkl.load(open(ck_path, 'rb'))
        else:
            truth = pkl.load(open(os.path.join(path, dev_file), 'rb'))
            truth = truth[:int(len(truth)*sample_ratio)]
            pkl.dump(truth, open(ck_path, 'wb'))
    else:
        truth = json.load(open(os.path.join(path, dev_file), 'r'))
        truth = truth[:int(len(truth)*sample_ratio)]
